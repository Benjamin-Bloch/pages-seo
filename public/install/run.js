...
import { mkdtempSync, rmSync, writeFileSync, readFileSync, existsSync, copyFileSync, openSync } from 'node:fs';
...
const REF = (process.env.PAGES_SEO_REF || 'v1.0.4').trim() || 'v1.0.4';
const REPO_TAG_TARBALL    = `https://github.com/Benjamin-Bloch/pages-seo/archive/refs/tags/${REF}.tar.gz`;
const REPO_BRANCH_TARBALL = `https://github.com/Benjamin-Bloch/pages-seo/archive/refs/heads/${REF}.tar.gz`;
...
async function askPassword(q) {
  ...
  return new Promise((resolve) => {
    ...
    let buf = '';
    let lastShown = false; // whether the last char on screen is plain
    const onData = (b) => {
      const s = b.toString('utf8');
      for (const ch of s) {
        if (ch === '\r' || ch === '\n') {
          // Mask the trailing plain char so the final prompt is all stars.
          if (lastShown) output.write('\b*');
        ...
        // Mask the previous plain char (if any), then show this one in
        // plain. Next keystroke will mask it too.
        if (lastShown) output.write('\b*');
        buf += ch;
        output.write(ch);
        lastShown = true;
...
async function ensureD1(name) {
  ...
  // Wrangler's output format has shifted across versions. Match both the
  // legacy TOML snippet (database_id = "uuid") and the current JSON
  // snippet ("database_id": "uuid"); fall back to any bare UUID.
  let m = all.match(/"?database_id"?\s*[=:]\s*"([0-9a-f-]{36})"/);
  if (!m) m = all.match(/\b([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\b/);
...
function fetchSource(workDir) {
  ...
  log(`  downloading pages-seo source (${REF})…`);
  // Try the tag first (the common case), fall back to a branch name
  // so PAGES_SEO_REF=main / PAGES_SEO_REF=feature-x still works.
  let dl = run('curl', ['-fsSL', '-o', tar, REPO_TAG_TARBALL], { stdio: 'inherit' });
  if (dl.status !== 0) {
    dl = run('curl', ['-fsSL', '-o', tar, REPO_BRANCH_TARBALL], { stdio: 'inherit' });
  }
  if (dl.status !== 0) die(`Failed to download source for ref "${REF}". Set PAGES_SEO_REF=main for bleeding edge.`);
...
function patchWranglerToml(dir, project, d1Id, r2Name) {
  ...
  // the repo doesn't carry the maintainer's real D1/R2 IDs. If
  // wrangler.toml isn't there, materialise it from the template.
  if (!existsSync(path)) {
    const template = join(dir, 'wrangler.template.toml');
    if (existsSync(template)) copyFileSync(template, path);
    else die('No wrangler.toml or wrangler.template.toml in source archive.');
  }
