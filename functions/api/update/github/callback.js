// GET /api/update/github/callback?code=…&state=<nonce>:<userState>
//
...
// /api/update, and redirect back to /update?gh=connected&state=…
// so the SPA can re-render and continue.
...
function fail(url, code, msg) {
  // Redirect to /update with the error in a query param so the SPA
...
export const onRequestGet = async ({ env, request }) => {
  ...
  if (!env?.GITHUB_OAUTH_CLIENT_ID || !env?.GITHUB_OAUTH_CLIENT_SECRET) {
    return fail(url, 'oauth_not_configured');
  }
  const code = String(url.searchParams.get('code') || '');
  const rawState = String(url.searchParams.get('state') || '');
  if (!code)     return fail(url, 'missing_code');
  if (!rawState) return fail(url, 'missing_state');

  // state shape is `<32-hex-nonce>:<base64-user-state>`. Split on the
  // first colon only — user state may contain colons after base64
  // padding normalisation.
  const colon = rawState.indexOf(':');
  if (colon < 0) return fail(url, 'bad_state');
  const nonce = rawState.slice(0, colon);
  const userState = rawState.slice(colon + 1);

  const expected = readCookie(request, 'ps_gh_nonce');
  if (!expected || expected !== nonce) {
    return fail(url, 'nonce_mismatch', 'Cookie nonce missing or did not match. Try Connect GitHub again.');
  }
  if (!/^[0-9a-f]{32}$/.test(nonce)) return fail(url, 'bad_nonce');
  ...
  try {
    ...
    if (d?.error) return fail(url, d.error, d.error_description);
    token = String(d?.access_token || '');
    if (!token) return fail(url, 'no_token');
  } catch (e) {
    return fail(url, 'exchange_failed', String(e?.message || e));
  }
